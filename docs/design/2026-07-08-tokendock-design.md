...
RS256 JWTs, `kid` and `typ: at+jwt` (RFC 9068, disable with `rfc9068: false` / `TOKENDOCK_RFC9068=false`) headers. Claims: `iss`, `sub`, `aud`, `exp`, `iat`, `jti`, `scope` (space-delimited), merged with per-client custom claims. Default signing key: ephemeral RSA-2048 generated at startup; optional mounted PEM for reproducibility.
